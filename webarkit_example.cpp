...
std::shared_ptr<IRenderPipelineState> renderPipelineState_Quad_;
...
static void processVideo(webarkit::WebARKitManager &manager,
  ...
  if (!capture.isOpened()) { // This section prompt an error message if no video
                             // stream is found//
...
#if USE_OPENGL_BACKEND
  const char *title = "OpenGL WebARKit example";
#else
  const char *title = "Vulkan WebARKit example";
#endif
...
static void initIGL() {
  // create a device
  {
#if USE_OPENGL_BACKEND
#if IGL_PLATFORM_WIN
    auto ctx = std::make_unique<igl::opengl::wgl::Context>(
        GetDC(glfwGetWin32Window(window_)), glfwGetWGLContext(window_));
    ...
    auto ctx = vulkan::HWDevice::createContext(
        cfg, (void *)glfwGetWin32Window(window_));
#elif __APPLE__
    auto ctx = vulkan::HWDevice::createContext(
        cfg, (void *)glfwGetCocoaWindow(window_));
#elif defined(__linux__)
    auto ctx = vulkan::HWDevice::createContext(
        cfg, (void *)glfwGetX11Window(window_), 0, nullptr,
...
#error Unsupported OS
    ...
    device_ = vulkan::HWDevice::create(std::move(ctx), devices[0],
...
#if USE_OPENGL_BACKEND
  codeVS =
      std::regex_replace(codeVS, std::regex("gl_VertexIndex"), "gl_VertexID");
#endif

  desc.shaderStages = ShaderStagesCreator::fromModuleStringInput(*device_, codeVS.c_str(), "main", "", codeFS, "main", "", nullptr);
  renderPipelineState_Quad_ = device_->createRenderPipeline(desc, nullptr);
  IGL_ASSERT(renderPipelineState_Quad_);
}
...
static void render(const std::shared_ptr<ITexture>& nativeDrawable) {
  ...
  commands->bindRenderPipelineState(renderPipelineState_Quad_);
  commands->bindViewport(viewport);
  commands->bindScissorRect(scissor);
  commands->pushDebugGroupLabel("Render Quad", igl::Color(1, 0, 0));
  commands->draw(PrimitiveType::Triangle, 0, 6);
